...
export const getBadgeRequirements = async (_req: Request, res: Response): Promise<void> => {
  try {
...
export const updateGamificationStats = async (req: Request, res: Response): Promise<void> => {
  try {
    const userId = req.user?._id;
    if (!userId) {
      res.status(401).json({ message: 'Unauthorized' });
...
export const resetStreak = async (req: Request, res: Response): Promise<void> => {
  try {
    const userId = req.user?._id;
    if (!userId) {
      res.status(401).json({ message: 'Unauthorized' });
