...
let userId: string;
let io: Server;
let serverSocket: Server['sockets'];
...
describe('Gamification Real-time Updates', () => {
  test('should emit gamificationUpdate event when points are updated', async () => {
    ...
    const updatePromise = new Promise<void>((resolve) => {
      clientSocket.on('gamificationUpdate', (data: GamificationUpdate) => {
  ...
  test('should only emit events to the correct user', async () => {
    ...
    const otherSocket = SocketIOClient(`http://localhost:${(httpServer.address() as AddressInfo).port}`, {
      auth: { userId: otherUser._id.toString() },
      withCredentials: true
    });

    // Set up event listeners for both sockets
    const originalUserPromise = new Promise<void>((resolve) => {
      clientSocket.on('gamificationUpdate', (data: GamificationUpdate) => {
    ...
    const otherUserPromise = new Promise<void>((resolve, reject) => {
