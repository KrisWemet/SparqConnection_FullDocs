...
export async function analyzeAnalytics(startDate, endDate, specificJourneyId) {
    ...
    for await (const line of rl) {
        try {
            ...
            if (specificJourneyId && logEntry.metadata?.journeyId !== specificJourneyId)
                continue;
...
function updateTimeRangeStats(stats, timestamp) {
...
function handleJourneyStart(result, logEntry) {
...
function handleDayComplete(result, logEntry) {
...
function handleActivityComplete(result, logEntry) {
    ...
    if (completionStatus) {
        stats.successRates[completionStatus]++;
    }
}
function handleSkippedActivity(result, logEntry) {
...
function calculateFinalStats(result) {
...
export function generateAnalyticsReport(results) {
